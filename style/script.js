...
// --- Mã hiển thị Thư mời kỷ yếu (mở bằng nút + hiệu ứng) ---
(function(){
  const overlay = document.getElementById('invitation-overlay');
  const modal = document.getElementById('invitation-modal');
  const btnClose = document.getElementById('invitation-close');
  const btnAccept = document.getElementById('invitation-accept');
  const btnLater = document.getElementById('invitation-later');
  ...
  function showInvitation(){
    ...
    // focus primary action for accessibility after animation start
    setTimeout(()=> btnAccept?.focus(), 220);
  }
  ...
  function hideInvitation(){
    ...
    // wait for modal animation to finish, then hide overlay
    const onEnd = (e)=>{
  ...
  btnClose?.addEventListener('click', hideInvitation);
  btnLater?.addEventListener('click', hideInvitation);
  overlay.addEventListener('click', (e)=>{
    if (e.target === overlay) hideInvitation();
  });
  btnAccept?.addEventListener('click', ()=>{
    try { localStorage.setItem('invitationAccepted', '1'); } catch(e) {}
    hideInvitation();
  });
  btnOpen?.addEventListener('click', showInvitation);
...
// --- Hiển thị Thư cảm ơn / Rất tiếc sau khi tương tác với modal ---
(function(){
  const invOverlay = document.getElementById('invitation-overlay');
  const invModal = document.getElementById('invitation-modal');
  const btnAccept = document.getElementById('invitation-accept');
  const btnLater = document.getElementById('invitation-later');

  const respOverlay = document.getElementById('response-overlay');
  const respModal = document.getElementById('response-modal');
  const respTitle = document.getElementById('response-title');
  const respBody = document.getElementById('response-body');
  const respClose = document.getElementById('response-close');
  const respOk = document.getElementById('response-ok');

  if (!respOverlay || !respModal) return;

  function showOverlay(el, modalEl) {
    el.classList.remove('hidden', 'is-hiding');
    void el.offsetWidth;
    el.classList.add('is-visible');
    el.setAttribute('aria-hidden', 'false');
    document.body.style.overflow = 'hidden';
    setTimeout(()=> modalEl?.querySelector('button, a, [tabindex]')?.focus(), 220);
  }
  function hideOverlay(el, modalEl) {
  ...
  function showResponse(title, html) {
    // ẩn invitation nếu đang mở
    if (invOverlay && !invOverlay.classList.contains('hidden')) {
      hideOverlay(invOverlay, invModal);
    }
    respTitle.textContent = title;
    respBody.innerHTML = html;
    showOverlay(respOverlay, respModal);
  }

  // Xử lý nút từ modal chính
  btnAccept?.addEventListener('click', () => {
    try { localStorage.setItem('invitationAccepted', '1'); } catch(e) {}
    showResponse('Lời cảm ơn❤️', `
      <p>Cảm ơn bạn đã xác nhận tham gia!</p>
      <p>Chúng mình rất mong được gặp bạn tại buổi kỷ yếu.</p>
  ...
  // Đóng response
  respClose?.addEventListener('click', () => hideOverlay(respOverlay, respModal));
  respOk?.addEventListener('click', () => hideOverlay(respOverlay, respModal));
